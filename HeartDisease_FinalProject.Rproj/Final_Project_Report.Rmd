...
outlier_0 <- data$trestbps[!is.na(data$trestbps) & (data$trestbps == 0)]
outlier_0
...
# numeric variables
counts <- table(data$sex, data$isHeartDisease)

barplot(counts,
        beside = TRUE,
        col = c("pink", "lightblue"),
        main = "Sex Distribution by Heart Disease Status",
...
# age
#histogram
hist(data$age[data$isHeartDisease == "No Disease"], main = "Age (No Heart Disease)", xlab = "age")
hist(data$age[data$isHeartDisease == "Heart Disease"], main = "Age (Heart Disease)", xlab = "age")
...
# Convert categorical variables to factor with proper labels if needed
data$cp <- factor(data$cp)
data$fbs <- factor(data$fbs, labels = c("<=120", ">120"))
data$restecg <- factor(data$restecg)
data$exang <- factor(data$exang, labels = c("No", "Yes"))
data$isHeartDisease <- factor(data$isHeartDisease, labels = c("No Disease", "Heart Disease"))

# --- Chest Pain Type (cp) ---
ggplot(data, aes(x = cp, fill = isHeartDisease)) +
  geom_bar(position = "dodge") +
  labs(title = "Chest Pain Type by Heart Disease", x = "Chest Pain Type", y = "Count", fill = "Heart Disease") +
  scale_fill_manual(values = c("No Disease" = "lightblue", "Heart Disease" = "pink")) +
  theme_minimal()

# --- Fasting Blood Sugar (fbs) ---
ggplot(data, aes(x = fbs, fill = isHeartDisease)) +
  geom_bar(position = "dodge") +
  labs(title = "Fasting Blood Sugar by Heart Disease", x = "FBS", y = "Count", fill = "Heart Disease") +
  scale_fill_manual(values = c("No Disease" = "lightblue", "Heart Disease" = "pink")) +
  theme_minimal()

# --- Resting ECG (restecg) ---
ggplot(data, aes(x = restecg, fill = isHeartDisease)) +
  geom_bar(position = "dodge") +
  labs(title = "Resting ECG by Heart Disease", x = "Resting ECG", y = "Count", fill = "Heart Disease") +
  scale_fill_manual(values = c("No Disease" = "lightblue", "Heart Disease" = "pink")) +
  theme_minimal()

# --- Exercise-Induced Angina (exang) ---
ggplot(data, aes(x = exang, fill = isHeartDisease)) +
  geom_bar(position = "dodge") +
  labs(title = "Exercise-Induced Angina by Heart Disease", x = "Exang", y = "Count", fill = "Heart Disease") +
  scale_fill_manual(values = c("No Disease" = "lightblue", "Heart Disease" = "pink")) +
  theme_minimal()
